...
<div class="card card-modern">
    <div class="card-modern-header">
        <span><i class="bi bi-image"></i> Sửa Banner</span>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="product_id" id="product_id" class="form-select form-select-modern">
                    ...
                    @foreach ($products as $product)
                        <option value="{{ $product->id_product }}" {{ old('product_id', $banner->product_id) == $product->id_product ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                @if ($banner->image)
                    <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->name }}" width="100" class="mt-2">
                @endif
                @error('image')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <button type="submit" class="btn btn-add-modern">Cập nhật Banner</button>
        </form>
    </div>
</div>
@endsection
